...
public abstract class TimerJobFactoryType {
    ...
    public static final TimerJobFactoryType MAPDB = new TimerJobFactoryType("mapdb") {
        public TimerJobFactoryManager createInstance() {
            try {
                return (TimerJobFactoryManager)Class.forName("org.drools.persistence.mapdb.MapDBTimeJobFactoryManager").newInstance();
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
